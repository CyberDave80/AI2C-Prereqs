...
class Player:
    ...
    def visit_planet(self, planet: Planet):
        self.visited_planets.add(planet)
        self.distance_traveled += abs(self.current_planet - planet)
        self.current_planet = planet

    def buy_fuel(self, spacecraft, price_per_unit=2.0):
        print(f"You have {self.credits} credits.")
        try:
            amount = float(
                input(f"Enter fuel amount to buy in kilounits (price: {price_per_unit}/kilounit): ")
            )
        except ValueError:
            print("Invalid input.")
            return
    ...
    def complete_mission(self, planet):
